...
export default function NewUser() {
  const { sendService, isLoading, hasError, response } = useService(newUser)
    ...
    <div className="flex h-screen justify-center overflow-auto bg-[#1b1c23] p-10">
      <div className="block max-w-lg w-full">
        ...
        <h2 className="text-blanco1 font-light mt-4">
          Darás de alta un nuevo <strong>usuario</strong>.
        </h2>
        ...
        <div className="bg-white p-4 pt-6 rounded-md shadow-xl">
          {response ? (
            <>
              ...
              <p className="text-negro2 mt-4 mb-2">
                Email: <strong>{response.data.correo_electronico}</strong>
              </p>
              <p className="text-negro2 mt-4 mb-2">
                Nombre y Apellido:{" "}
                <strong>
        ...
        {hasError && (
          <div className="text-rojo mt-4 text-center">{hasError.message}</div>
        )}
      </div>
    </div>
  )
