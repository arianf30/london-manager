          ...
          <div className="w-[351px] m-auto py-10">
            ...
            <div className="rounded-md drop-shadow-xl text-negro1 select-none">
              <div className="inline-block bg-white w-full h-auto rounded-t-md px-5 py-6">
                <h4 className="mt-2 mb-1"><strong>Iniciar sesión</strong></h4>
                <p className="text-sm">¿No tenés cuenta? <Link href="/signup"><a className="text-sm text-violeta underline-offset-2 hover:underline">Registrarte</a></Link> es muy fácil.</p>
                ...
                >
                  {props => (
                    <form onSubmit={props.handleSubmit}>
                      <div className="mt-7">
                        <input type="text" onChange={props.handleChange} value={props.values.correo_electronico} name="correo_electronico" id="correo_electronico" className="w-full text-sm h-[40px] px-5 rounded-sm bg-blanco3 border-2 border-gris1 focus:border-violeta focus:outline-none appearance-none placeholder-negro3 text-negro3 transition ease-in duration-150" placeholder="Correo electrónico" />
                      </div>

                      <div className="relative mt-2">
                        <input type={showPass ? "text" : "contrasena"} onChange={props.handleChange} value={props.values.contrasena} name="contrasena" id="contrasena" className="w-full text-sm h-[40px] px-5 rounded-sm bg-blanco3 border-2 border-gris1 focus:border-violeta focus:outline-none appearance-none placeholder-negro3 text-negro3 transition ease-in duration-150" placeholder="Contraseña" />
                      ...
                      <div className="mt-4">
                        {props.isSubmitting
                          ? <div className="flex items-center justify-center text-blanco1 h-[40px] text-center w-full bg-gris2 rounded-sm">Ingresando...</div>
                          : <button type="submit" className="text-blanco1 h-[40px] text-center w-full bg-gradient-to-r from-violeta hover:from-verde to-violeta rounded-sm hover:drop-shadow-md transition ease-linear duration-300">Ingresar</button>}
                      </div>
                    </form>
                  )}
                </Formik>
                ...
                <div className="mt-4 text-center">
                  <Link href="/recuperar_clave"><a className="text-sm text-violeta underline-offset-2 hover:underline">No recuerdo mi contraseña</a></Link>
                </div>
              </div>
              ...
              <div className="inline-block bg-white w-full h-auto rounded-b-md px-5 py-6">
                <button onClick={() => signIn('google')} className="flex h-[40px] items-center justify-center w-full text-center border-2 border-gris2 rounded-md hover:bg-blanco3">
                  <div className="w-6 h-6 mr-3 bg-[url('/img/system/icons/google.svg')] bg-no-repeat bg-contain bg-center"></div>
                  <p className="text-xs">Iniciá sesión con tu cuenta de Google</p>
                </button>
              </div>
            </div>
          </div>
