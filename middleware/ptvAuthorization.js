...
const ptvAuthorization = (handler) => async (req, res) => {
  if (req.query.ptv) {
    const userPtvs = await UserModel.findOne({ attributes: ['ptos_vta'], where: { id: req.userId } })
    ...
    if (encontro.length) {
      ...
      const findJerarquia = await UserPtvModel(req.query.ptv).findOne({ attributes: ['id_jerarquia'], where: { id_usuario: req.userId } })
      const findPermissions = await JerarquiaModel(req.query.ptv).findOne({ attributes: ['denominacion', 'jerarquia'], where: { id: findJerarquia.id_jerarquia } })
      const dataSections = await SeccionModel.findAll({ attributes: ['id', 'tipo', 'nombre', 'icono'] })
